...
Here are a variety of notes about contributing to the NynjaCB codebase.  This isn't stuff that applies if you are simply integrating NynjaCB in your site.
...
There are [some notes in the style guide](https://github.com/ianb/javascript#modules).  NynjaCB uses [requirejs](http://requirejs.org/) for module loading, and the AMD pattern generally.  Each module should go in `app/http/nynjacb/public/` and look like:
...
The first list is the dependencies (modules) you need.  If you need to require module modules later (lazily, or later than load time due to circular dependencies) you must include `require` among your dependencies.  There *is* a global `require()` object, but you can't use it, because NynjaCB uses a [context](http://requirejs.org/docs/api.html#multiversion).
...
```javascript
session = require({context: "nynjacb"})("session");
```
...
"Templates" are just elements that are cloned out of `interface.html`.  They generally have an id like `nynjacb-template-foo`, and you'd clone them by doing `templating.sub("foo", {vars})`.
...
```html
<button id="nynjacb-collapse-foo" data-toggles="#nynjacb-expand-foo">-</button>
<button id="nynjacb-expand-foo" data-toggles="#nynjacb-collapse-foo" style="display: none">+</button>
```

Then use `ui.displayToggle("#nynjacb-expand-foo")` to show the expand button, and the collapse button will be automatically hidden.  Note that the selector in `data-toggles` can be inclusive of the element itself (everything matching selector will be hidden, except the element itself).
...
We do most of our planning in the [Github issue tracker](https://github.com/mozilla/nynjacb/issues), and make use of the [Milestones](https://github.com/mozilla/nynjacb/issues/milestones) (more than labels).  You are welcome to just submit issues without worrying about this, but if you are looking at a ticket and want how it relates to our plans then you might want to know our system.
...
* [Next Tasks](https://github.com/mozilla/nynjacb/issues?milestone=17&state=open): this is a long-standing milestone that represents tasks we want to do soon, but not in the current milestone.  Typically when planning the next iteration we'll look through this milestone and pick out issues.  If something in this milestone is a priority for you, please note that in a comment on the ticket so we can understand your needs.

* [Blue Sky](https://github.com/mozilla/nynjacb/issues?milestone=23&state=open): another long-standing milestone, this represents stuff we'd like to do but don't have any plans to do any time soon.  If you are looking to contribute this is an excellent place to look for ideas.  And if it's something you think would be helpful to you, a comment would be good -- especially one that outlines a use case.

* [Issues with no milestone](https://github.com/mozilla/nynjacb/issues?milestone=none&page=1&state=open): these are issues that haven't been triaged.  We try to assign milestones every week to these issues.  If we don't then probably we are caught up in something we need to focus on over a short period, or we're just all out on vacation or something.
