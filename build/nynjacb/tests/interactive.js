...
if (config) {
  ...
  window.NynjaCBConfig = config;
  for (var a in config) {
    NynjaCB.config(a, config[a]);
  }
}
...
el.find("#idle-check").change(function (event) {
  if (event.target.checked) {
    NynjaCBTestSpy.setIdleTime(100);
  } else {
    NynjaCBTestSpy.setIdleTime(3*60*1000);
  }
});

el.find("#expire-check").change(function (event) {
  if (event.target.checked) {
    NynjaCBTestSpy.setByeTime(10*1000);
  } else {
    NynjaCBTestSpy.setIdleTime(10*60*1000);
  }
});
...
if (NynjaCB.config.get("includeHashInUrl")) {
...
Test.addControl($('<input placeholder="Tool name">').keypress(function (event) {
  var el = $(event.target);
  if (event.which == 13) {
    NynjaCB.config("toolName", el.val() || null);
    el.val("");
  }
}));
